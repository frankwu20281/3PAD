...
def create_sprites ( **kwargs): 
        image = None 
        try: 
            path = f"sprites\Textures\{kwargs['name']}.png"
            image = PhotoImage(file=path)
        except Exception: 
            print('load error')
            pass
        return image
def add_image ( sprite_name, key:str, x:int, y:int,parent=None): 
    try:
        if not parent:
            image = parent.create_image(x,y,anchor = "nw", image=create_sprites(name = sprite_name))
            return image
    except: pass
...
class Game(Tk):
    def __init__(self):
        ...
        for key, value in self.terrain_cords_dict.items():
            self.sprite_dict[key] = create_sprites(name = key)
        self.sprite_dict['character'] = create_sprites(name= 'character')
        #self.sprite_dict['grass'] = create_sprites(name= 'grass')

        
        #self.character_sprite = create_sprites(name ='character')
        ...
        #maybe for monster loop but most likey will not use this way to run it
    ...
    def movement_loop(self):
        ...
        if self.movement_allowed:
            ...
            self.new_frame()
            self.after(33, lambda : self.movement_loop())
    ...
    def monster_movement (self): 
        for key, list in game_instance.monster_movement_cords_dict.items():  
            for monster_cord in game_instance.monster:
                ...
                if (monster_cord[0]//GRID_SIZE,monster_cord[1]//GRID_SIZE)  in list:

                    new_monster_cord = {"down": (monster_cord[0],monster_cord[1]+1),
                                "up": (monster_cord[0],monster_cord[1]-1),
                                "left": (monster_cord[0]-1,monster_cord[1]),
                                "right": (monster_cord[0]+1,monster_cord[1])}.get(key)
        ...
        for key, list in game_instance.terrain_cords_dict.items():
            if key != "grass":
                for value in list: 
                    ...
                    if new_x in range(obj_x-GRID_SIZE, obj_x +GRID_SIZE) and new_y in range(obj_y-GRID_SIZE, obj_y +GRID_SIZE) and key != "grass" and (abs(obj_y -new_y)!=GRID_SIZE ) and abs(obj_x -new_x)!=GRID_SIZE:
    ...
    def new_frame(self):
        ...
        for key, list in self.terrain_cords_dict.items():
            for value in list: 
                if value[0] in range(top_left_x -GRID_SIZE, top_left_x +WINDOW_SIZE_WIDTH+1) and value[1] in range(top_left_y-GRID_SIZE, top_left_y + WINDOW_SIZE_HEIGHT+1)and key!= 'grass':
                        self.canvas.create_image(  value[0]- top_left_x , value[1]-top_left_y, 
                                anchor='nw', image=self.sprite_dict[key])
          
        self.canvas.create_image(10 * GRID_SIZE, 7 * GRID_SIZE, 
                                 anchor=NW, image=self.sprite_dict['character'])
...
class Map():
    ...
    def reset_cord_storage (self):
        ...
        game_instance.terrain_cords_dict = {"wall":game_instance.wall, 
                                   ...
                                   "grass":game_instance.grass_full, 
    ...
    def terrain_load(self):
        ...
        for x_line in game_instance.map_storage[str(game_instance.level_number)]:
            ...
            y_cord+=1
        print(game_instance.terrain_cords_dict)
